...
def mesch_provision_adv(uuid, addr, attention_duration):
    logging.debug("%s", mesch_provision_adv.__name__)
...
def mesh_cfg_model_pub_va_set(net_idx, addr, el_address, model_id, pub_virt_address, appkey_index,
                                 cred_flag, ttl, period, transmit):
    ...
    payload = binascii.unhexlify(pub_virt_address)
    payload_len = len(pub_virt_address)

    data = bytearray(struct.pack("<HHHHHBBBBB", net_idx, addr, el_address, model_id, appkey_index,
                                 cred_flag, ttl, period, transmit, payload_len))
    data.extend(payload)
...
def mesh_cfg_model_sub_va_add(net_idx, addr, el_address, model_id, virt_address):
    logging.debug("%s", mesh_cfg_model_sub_va_add.__name__)

    stack = get_stack()
    iutctl = get_iut()

    payload = binascii.unhexlify(virt_address)
    payload_len = len(virt_address)

    data = bytearray(struct.pack("<HHHHB", net_idx, addr, el_address, model_id, payload_len))
    data.extend(payload)
...
def mesh_cfg_model_sub_va_del(net_idx, addr, el_address, model_id, virt_address):
...
def mesh_cfg_model_sub_va_ovw(net_idx, addr, el_address, model_id, virt_address):
...
def mesh_cfg_lpn_polltimeout_get(net_idx, addr, unicast_addr):
    ...
    (timeout,) = struct.unpack_from('<H', rsp)
