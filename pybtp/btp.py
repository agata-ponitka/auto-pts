...
def mmdl_gen_lvl_set(lvl, tt=None, delay=None, ack=True):
    ...
    if ack == True:
        ...
        if delay is not None:
            ...
            stack.mesh.rcv_status_data_set("Status", [level])
        logging.debug("Status level = %r", stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_gen_dtt_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [tt])
    logging.debug("Status tt = %r", stack.mesh.rcv_status_data_get('Status'))
...
def mmdl_gen_plvl_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [power_level])
    logging.debug("Status power level = %r", stack.mesh.rcv_status_data_get('Status'))
...
def mmdl_gen_plvl_set(power_lvl, tt=None, delay=None, ack=True):
    ...
    if ack == True:
        ...
        logging.debug("Status power = %r", stack.mesh.rcv_status_data_get('Status'))
...
def mmdl_gen_plvl_last_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [power_last])
    logging.debug("Status power last = %r", stack.mesh.rcv_status_data_get('Status'))
...
def mmdl_gen_plvl_dflt_set(dflt, ack=True):
    ...
    if ack == True:
        ...
        stack = get_stack()
        stack.mesh.rcv_status_data_set("Status", [power_default])
        logging.debug("Status power = %r", stack.mesh.rcv_status_data_get('Status'))
...
def mmdl_gen_plvl_range_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [status, range_min, range_max])
    logging.debug("Status: Range Min = %r Range Max = %r Status = %r", range_min, range_max, status)
...
def mmdl_gen_battery_get():
    ...
    hdr_fmt = '<B3s3sB'
    battery, discharge_min, charge_min, flags = struct.unpack_from(hdr_fmt, rsp)
...
def mmdl_sensor_desc_get(sensor_id=None):
    ...
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['sensor_desc_get'], data=data)
...
def mmdl_sensor_get(sensor_id):
    ...
    if len(binascii.hexlify(rsp)) == 6:
        ...
        hdr_fmt = '<H1sH3sH3s'
        sensor_id_0, sensor_data_0, sensor_id_1, sensor_data_1, sensor_id_2, sensor_data_2 = struct.unpack_from(hdr_fmt, rsp)
...
def mmdl_sensor_cadence_get(sensor_id):
    ...
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['sensor_cadence_get'], data=data)
    hdr_fmt = '<H'
...
def mmdl_sensor_settings_get(sensor_id):
    ...
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['sensor_settings_get'], data=data)
    hdr_fmt = '<HH'
...
def mmdl_sensor_series_get(sensor_id, raw_values):
    ...
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['sensor_series_get'], data=data)
    hdr_fmt = '<9sH'
...
def mmdl_time_get():
    ...
    hdr_fmt = '<5sBBHB'
    (tai, subsecond, uncertainty, tai_utc_delta, time_zone_offset) = struct.unpack_from(hdr_fmt, rsp)
    tai = int(binascii.hexlify(tai[::-1]), 16)
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [tai, subsecond, uncertainty, tai_utc_delta, time_zone_offset])
...
def mmdl_time_role_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [role])
...
def mmdl_time_zone_set(new_offset, timestamp):
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [new_offset, timestamp])
    logging.debug('Status: New offset = %r Timestamp = %r', new_offset, timestamp)
...
def mmdl_time_tai_utc_delta_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [delta_current, delta_new, timestamp])
    logging.debug('Status: Delta current = %r New delta = %r Timestamp = %r', delta_current, delta_new, timestamp)
...
def mmdl_time_tai_utc_delta_set(delta_new, timestamp):
    ...
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['time_tai_utc_delta_set'], data=data)

    hdr_fmt = '<hQ'
    delta_new, timestamp = struct.unpack_from(hdr_fmt, rsp)
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [delta_new, timestamp])
    logging.debug('Status: New delta = %r Timestamp = %r', delta_new, timestamp)
...
def mmdl_light_lightness_set(lightness, tt=None, delay=None, ack=True):
    ...
    if ack == True:
        ...
        logging.debug('Status: Lightness = %r ', stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_light_lightness_linear_get():
    ...
    logging.debug('Status: Lightness linear = %r ', stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_light_lightness_last_get():
    ...
    logging.debug('Status: Lightness last = %r ', stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_light_lightness_default_get():
    ...
    iutctl = get_iut()
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['light_lightness_default_get'])

    hdr_fmt = '<H'
    dflt, = struct.unpack_from(hdr_fmt, rsp)
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [dflt])

    logging.debug('Status: Lightness default= %r ', stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_light_lc_mode_get():
    ...
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [mode])
...
def mmdl_light_lc_occupancy_mode_get():
    ...
    iutctl = get_iut()
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['light_lc_occupancy_mode_get'])

    hdr_fmt = '<B'
    occupancy_mode, = struct.unpack_from(hdr_fmt, rsp)
    stack = get_stack()
    stack.mesh.rcv_status_data_set('Status', [occupancy_mode])
...
def mmdl_light_lc_light_onoff_mode_get():
    ...
    iutctl = get_iut()
    (rsp,) = iutctl.btp_socket.send_wait_rsp(*MMDL['light_lc_light_onoff_mode_get'])
...
def mmdl_light_lc_light_onoff_mode_set(light_onoff_mode, tt=None, delay=None, ack=True):
    ...
    if ack == True:
        ...
        logging.debug('Status: Lightness onoff mode= %r ', stack.mesh.rcv_status_data_get("Status"))
...
def mmdl_light_lc_property_get(prop_id):
    ...
    logging.debug('Status: Property value = %r , Property Id = %r ', prop_val, prop_id)
