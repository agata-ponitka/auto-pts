...
def hdl_wid_33(desc):
    ...
    btp.mesch_provision_adv(uuid, addr, attention_duration)
    return True
...
def hdl_wid_514(desc):
    ...
    if "Config Model Subscription" in desc:
        sub_address = 0x0003
        model_id = 0x0000
        virt_address = '00000000000000000000000000000001'

        if "Virtual Address Add" in desc:
            stack.mesh.model_data = [stack.mesh.el_address, int(virt_address, 16), model_id]
    ...
    if "Config Vendor Model Subscription Get" in desc:
        model_id = 0x1234
        cid = 0x05F1
        stack.mesh.model_data = [stack.mesh.el_address, cid | model_id << 16]

        btp.mesh_cfg_model_sub_vnd_get(stack.mesh.net_idx, stack.mesh.addr, stack.mesh.el_address, model_id, cid)
        return stack.mesh.status == 0x00
    ...
    if "Config SIG Model Subscription Get " in desc:
        ...
        btp.mesh_cfg_model_sub_get(stack.mesh.net_idx, stack.mesh.addr, stack.mesh.el_address, model_id)
        return stack.mesh.status == 0x00
...
def hdl_wid_650(desc):
    ...
    if 'Config Model Publication VIRTUAL ADDRESS SET' in desc:
        ...
        el_address = int(params.get('Element Address'), 16)
        pub_addr = '00000000000000000000000000000001'
        appkey_index = int(params.get('AppKey Index'), 16)
        cred_flag = int(params.get('Credential Flag'), 16)
        ttl = int(params.get('Publish TTL'), 16)
        period = int(params.get('Publish Period'), 16)
        ...
        btp.mesh_cfg_model_pub_va_set(stack.mesh.net_idx, stack.mesh.addr, el_address, model_id, pub_addr,
                                         appkey_index, cred_flag, ttl, period, transmit)
        logging.debug("Status = 0x%2x", stack.mesh.status)
        return True # status is not checked because PTS returned status != 0x00
